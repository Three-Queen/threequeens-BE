        ...
        <form method="POST" action="{{ route('admin.portofolio.store') }}" enctype="multipart/form-data">
                ...
                <div>
                    ...
                    <select name="produk_ids[]" id="produk_ids" multiple placeholder="Pilih produk..."
                        ...
                        @foreach($produks as $produk)
                            <option value="{{ $produk->id }}" {{ is_array(old('produk_ids')) && in_array($produk->id, old('produk_ids')) ? 'selected' : '' }}>
                                {{ $produk->nama_produk }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="mb-5">
                ...
                <div class="relative">
                    ...
                    <input type="text" name="durasi_pengerjaan" value="{{ old('durasi_pengerjaan') }}" placeholder="Contoh: 2 Bulan, 14 Hari..."
            ...
            <div class="mb-8" x-data="galleryUpload()">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-4" x-show="files.length > 0" style="display: none;">
                    <template x-for="(file, index) in files" :key="index">
                        <div class="relative group rounded-xl overflow-hidden bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
                            <img :src="file.preview" class="w-full h-24 object-cover">
                            <button type="button" @click.prevent="removeFile(index)" class="absolute top-2 right-2 w-7 h-7 bg-red-500 hover:bg-red-600 rounded-full flex items-center justify-center text-white opacity-0 group-hover:opacity-100 transition-opacity shadow-sm">
                ...
                <div class="relative border-2 border-dashed border-gray-200 dark:border-gray-600 rounded-xl p-4 hover:border-primary transition-colors bg-gray-50 dark:bg-gray-800/50"
                    ...
                    <div class="text-center pointer-events-none">
                        ...
                        <p class="text-xs text-gray-400 mt-1" x-text="files.length > 0 ? files.length + ' file dipilih' : 'Belum ada file dipilih'"></p>
                    </div>
                </div>
    ...
    document.addEventListener('alpine:init', () => {
        Alpine.data('galleryUpload', () => ({
            ...
            handleFiles(newFiles) {
                ...
                // Then clear input so clicking same file works
